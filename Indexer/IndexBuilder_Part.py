...
def createIndex(termlists):
  ...
  for line in termlists.keys():
    ...
    print "index " + str(i) + "z: " + str(len(termlists.keys()))  
    i = i+1  
  return total  
  ...
  for key in termlists.keys():
    for word in termlists[key].keys():
      if word in reversedIndex.keys():
        ...
        reversedIndex[word] = {key: termlists[key][word]}
    print "key " + str(i) + "z: " + str(len(termlists.keys()))  
    i = i+1 
...
data_tickety = s.execute('''SELECT TicketID, Subject, Description, Solution FROM TicketData WHERE rowid BETWEEN 1 and 100''')  
...
ofile = open('indexTicketsPart.json', 'wb')
...
with open('indexTicketsPart.json', 'rb') as files:
